...
# <翻译开始>
	...
	//
	// 这里的注释解释了如何连接到DM8数据库。`dm:	//` 是驱动的URL格式，`userName` 和 `password` 是用于身份验证的用户名和密码，`ip:port` 是数据库服务器的地址和端口，`dbname` 是要连接的数据库名。在第二个示例中，`DW` 可能是分布式仓库的标识，后面跟着一个列表 `(192.168.1.1:5236,192.168.1.2:5236)`，表示数据库集群的多个节点。 md5:15e6de4613ebd611
# <翻译结束>
...
# <翻译开始>
	// 时区设置演示：
	// &loc=亚洲/上海 md5:dcdf1db7d830174b
# <翻译结束>
