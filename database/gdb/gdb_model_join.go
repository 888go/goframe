...
// Model("user", "u").LeftJoin("SELECT xxx FROM xxx", "a", "a.uid=u.uid") md5:5f7464280da64004
...
// Model("user", "u").RightJoin("SELECT xxx FROM xxx","a", "a.uid=u.uid") md5:dbab2528fb37c84e
...
// Model("user", "u").InnerJoin("SELECT xxx FROM xxx","a", "a.uid=u.uid") md5:eda419ad685c559d
...
// Model("order").LeftJoinOnField("product", "product_id") md5:5ace5bdef45c73d6
...
// Model("order").InnerJoinOnField("product", "product_id") md5:ac8281e2d383e3d6
...
// 例如：
// Model("order").InnerJoinOnField("user", "user_id")
// Model("order").InnerJoinOnField("product", "product_id") md5:bdc954b5bcb8a9c5
...
// Model("user").LeftJoinOnFields("order", "id", ">", "user_id")
// Model("user").LeftJoinOnFields("order", "id", "<", "user_id") md5:90ce0e2226eb4b30
...
// User("user").RightJoinOnFields("order", "id", "<", "user_id")
//
// 这里，`RightJoinOnFields` 是一个 Go 代码中的函数，用于在查询数据库时执行右连接操作，并且允许用户自定义连接的字段和比较操作。第一个参数是模型名（如 "user"），接下来的参数包括要连接的表名、字段名以及连接条件（等于、大于或小于）。 md5:563f2b0f155fc829
...
// Model("user").InnerJoinOnFields("order", "id", "<", "user_id")
//
// 这段代码是在 Go 语言中定义了一个方法，用于在两个数据表之间执行内连接（InnerJoin），并允许用户指定连接的字段以及比较运算符。例如，`"user".InnerJoinOnFields("order", "id", "=", "user_id")` 表示连接 "user" 表和 "order" 表，通过 "id" 字段进行等号（=）匹配。其他示例展示了使用大于（>）和小于（<）运算符的情况。 md5:0499f4b5bbbc2016
...
// 相关问题讨论：
// https://github.com/gogf/gf/issues/1024 md5:7b792ce187933a04
...
// getTableNameByPrefixOrAlias 检查`prefixOrAlias`是否是某个表的别名，如果是，则返回该表的实际名称，否则直接返回`prefixOrAlias`。 md5:ab423b9e1e0ad0ca
