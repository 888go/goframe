...
func (r X行记录数组) X是否为空() bool {
	return r == nil || r.X取数量() == 0
}

// Len 返回结果列表的长度。
func (r X行记录数组) X取数量() int {
	return len(r)
}

// Size 是函数 Len 的别名。
func (r X行记录数组) Size别名() int {
	return r.X取数量()
}
...
func (r X行记录数组) X分割(数量 int) []X行记录数组 {
	if 数量 < 1 {
		return nil
	}
	length := len(r)
...
func (r X行记录数组) X取json() string {
	content, _ := json类.X创建(r.X取Map数组()).X取json文本()
	return content
}

// Xml 将`r`转换为XML格式的内容。
func (r X行记录数组) X取xml(根标记 ...string) string {
	content, _ := json类.X创建(r.X取Map数组()).X取xml文本(根标记...)
	return content
}

// List 将 `r` 转换为一个 List。
func (r X行记录数组) X取Map数组() Map数组 {
...
func (r X行记录数组) X取字段数组(字段名称 ...string) []X字段值 {
	array := make([]X字段值, len(r))
	if len(r) == 0 {
		return array
	}
	key := ""
...
// 注意，项值可能为切片类型。
func (r X行记录数组) X取字段Map泛型类(字段名称 string) map[string]X字段值 {
	var (
		s              string
		m              = make(map[string]X字段值)
...
func (r X行记录数组) X取字段MapStr(字段名称 string) map[string]Map {
...
func (r X行记录数组) X取字段MapInt(字段名称 string) map[int]Map {
	m := make(map[int]Map)
	for _, item := range r {
		if v, ok := item[字段名称]; ok {
			m[v.X取整数()] = item.X取Map()
...
func (r X行记录数组) X取字段MapUint(字段名称 string) map[uint]Map {
	m := make(map[uint]Map)
	for _, item := range r {
		if v, ok := item[字段名称]; ok {
...
func (r X行记录数组) RecordKeyStr(key string) map[string]X行记录 {
	m := make(map[string]X行记录)
	for _, item := range r {
		if v, ok := item[key]; ok {
			m[v.String()] = item
		}
...
func (r X行记录数组) RecordKeyInt(key string) map[int]X行记录 {
	m := make(map[int]X行记录)
	for _, item := range r {
		if v, ok := item[key]; ok {
			m[v.X取整数()] = item
...
func (r X行记录数组) RecordKeyUint(key string) map[uint]X行记录 {
	m := make(map[uint]X行记录)
	for _, item := range r {
		if v, ok := item[key]; ok {
			m[v.X取正整数()] = item
		}
...
func (r X行记录数组) X取数组结构体指针(结构体指针 interface{}) (错误 error) {
