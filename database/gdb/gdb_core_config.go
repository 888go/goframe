...
type ConfigNode struct {
	...
	User                 string        `json:"user"`//qm:账号  cz:User string `json:"user"                   // 身份验证用户名。 md5:bc205abcf7fb6329
	Pass                 string        `json:"pass"`//qm:密码  cz:Pass string `json:"pass"                   // 认证密码。 md5:325b61eb5935f198
	Name                 string        `json:"name"`//qm:名称  cz:Name string `json:"name"                   // 默认使用的数据库名称。 md5:13fed4b7ca845a3b
	Type                 string        `json:"type"`//qm:类型  cz:Type string `json:"type"`                   // 数据库类型：mysql，mariadb，sqlite，mssql，pgsql，oracle，clickhouse，dm。 md5:6b9f1786cf5a0d7a
	Link                 string        `json:"link"`//qm:自定义链接信息  cz:Link string `json:"link"`                   // （可选）所有配置的自定义链接信息，作为一个单个字符串。 md5:cb9784decaebf7c3
	Extra                string        `json:"extra"`//qm:额外  cz:Extra string `json:"extra"                  // （可选）根据注册的第三方数据库驱动程序提供的额外配置。 md5:6ba9ea91183a2b0c
	Role                 string        `json:"role"`//qm:节点角色  cz:Role string `json:"role"`                   // （可选，默认为"master"）节点角色，用于主从模式：master, slave。 md5:9645d0e7417ebf0c
	Debug                bool          `json:"debug"`//qm:调试模式  cz:Debug bool `json:"debug"                  // （可选）调试模式启用调试信息日志和输出。 md5:e292d7585b9505f9
	Prefix               string        `json:"prefix"`//qm:表前缀  cz:Prefix string `json:"prefix"`                 //（可选）表前缀。 md5:201acb7d8a3cfba7
	DryRun               bool          `json:"dryRun"`//qm:空跑特性  cz:DryRun bool `json:"dryRun"`                 // （可选） dry run，只执行 SELECT 语句，而不执行 INSERT/UPDATE/DELETE 语句。 md5:3983d4a8bb269d45
	Weight               int           `json:"weight"`//qm:负载均衡权重  cz:Weight int `json:"weight"                 // （可选）用于负载均衡计算的权重，如果只有一个节点则无效。 md5:6be8657f1809396b
	Charset              string        `json:"charset"`//qm:字符集  cz:Charset string `json:"charset"                // (可选，默认为 "utf8") 操作数据库时使用的自定义字符集。 md5:e63288ee7f2834e2
	Protocol             string        `json:"protocol"`//qm:协议  cz:Protocol string `json:"protocol"`               // （可选， 默认为 "tcp"）有关可用网络的更多信息，请参阅 net.Dial。 md5:96a17fcac4ef394d
	Timezone             string        `json:"timezone"`//qm:时区  cz:Timezone string `json:"timezone"               // （可选）设置时区，用于显示和解释时间戳。 md5:f1c54d7158bc5e89
	Namespace            string        `json:"namespace"`//qm:命名空间  cz:Namespace string `json:"namespace"              // （可选）某些数据库的命名空间。例如，在pgsql中，`Name` 用作 `catalog`，而 `NameSpace` 用作 `schema`。 md5:8dc2fa65d951a94e
	MaxIdleConnCount     int           `json:"maxIdle"`//qm:最大闲置连接数  cz:MaxIdleConnCount int `json:"maxIdle"`                //（可选）底层连接池的最大空闲连接配置。 md5:b1f10cf73af643a1
	MaxOpenConnCount     int           `json:"maxOpen"`//qm:最大打开连接数  cz:MaxOpenConnCount int `json:"maxOpen"`                //（可选）底层连接池的最大打开连接配置。 md5:bd8ec40915479f50
	MaxConnLifeTime      time.Duration `json:"maxLifeTime"`//qm:最大空闲时长  cz:MaxConnLifeTime time.Duration `json:"maxLifeTime"`            // （可选）连接在被关闭之前允许的最大空闲时间。 md5:b078efc5d928800a
	QueryTimeout         time.Duration `json:"queryTimeout"`//qm:查询超时时长  cz:QueryTimeout time.Duration `json:"queryTimeout"           // （可选）每个DQL查询的最大时间。 md5:4c57cb456fb2ee8b
	ExecTimeout          time.Duration `json:"execTimeout"`//qm:执行超时时长  cz:ExecTimeout time.Duration `json:"execTimeout"            // (可选) DML的最大执行时间。 md5:481b9bbde2c49552
	TranTimeout          time.Duration `json:"tranTimeout"`//qm:事务超时时长  cz:TranTimeout time.Duration `json:"tranTimeout"            //（可选）事务的最大执行时间。 md5:8e1cd855a33e2c6b
	PrepareTimeout       time.Duration `json:"prepareTimeout"`//qm:预准备SQL超时时长  cz:PrepareTimeout time.Duration `json:"prepareTimeout"         // （可选）准备操作的最大执行时间。 md5:14739995bdfab318
	CreatedAt            string        `json:"createdAt"`//qm:创建时间字段名  cz:CreatedAt string `json:"createdAt"              // （可选）自动填充创建时间的字段名。 md5:85fd52001481b733
	UpdatedAt            string        `json:"updatedAt"`//qm:更新时间字段名  cz:UpdatedAt string `json:"updatedAt              //（可选）自动填充更新日期时间的表字段名称。 md5:5d4aa50ffafb9f8e
	DeletedAt            string        `json:"deletedAt"`//qm:软删除时间字段名  cz:DeletedAt string `json:"deletedAt"`              //（可选）自动填充更新日期时间的表字段名称。 md5:5d4aa50ffafb9f8e
	TimeMaintainDisabled bool          `json:"timeMaintainDisabled"`//qm:禁用时间自动更新特性  cz:TimeMaintainDisabled bool `json:"timeMaintainDisabled"   // （可选）禁用自动时间保持功能。 md5:082a8741b2590acc
}
