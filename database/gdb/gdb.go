...
type DB interface {
	...
	Raw(rawSql string, args ...interface{}) *Model  //qm:原生SQL cz:Raw(rawSql string, args ...interface{})     //qm:原生SQL cz:Raw(rawSql string, args ...interface{})     
	...
	// Also see Core.Ctx.
	Ctx(ctx context.Context) DB  //qm:设置上下文并取副本 cz:Ctx(ctx context.Context)     //qm:设置上下文并取副本 cz:Ctx(ctx context.Context)     
	...
	DoInsert(ctx context.Context, link Link, table string, data List, option DoInsertOption) (result sql.Result, err error)                          //qm:原生sql行记录 cz:Result    sql.Result     //qm:底层插入 cz:DoInsert(ctx context.Context, link Link, table string, data List, option DoInsertOption)     // See Core.DoInsert.
	...
	GetArray(ctx context.Context, sql string, args ...interface{}) ([]Value, error)              //qm:原生SQL查询数组 cz:GetArray(ctx context.Context, sql string, args ...interface{})     // See Core.GetArray.
	...
	Transaction(ctx context.Context, f func(ctx context.Context, tx TX) error) error   //qm:事务 cz:Transaction(ctx context.Context, f func(ctx context.Context, tx TX)     //qm:事务 cz:Transaction(ctx context.Context, f func(ctx context.Context, tx TX) error)     // See Core.Transaction.
	...
	GetCtx() context.Context                                                                                   //qm:取上下文对象 cz:GetCtx() context.Context     //qm:取上下文对象 cz:GetCtx() context.Context     // See Core.GetCtx.
	...
	Tables(ctx context.Context, schema ...string) (tables []string, err error)                                //qm:取表名称数组 cz:Tables(ctx context.Context, schema ...string)     // See Core.Tables. The driver must implement this function.
...
type TX interface {
	...
	GetStructs(objPointerSlice interface{}, sql string, args ...interface{}) error //qm:原生SQL查询到结构体数组指针 cz:GetStructs(objPointerSlice interface{}, sql string, args ...interface{})     
...
type DoCommitOutput struct {
	...
	Records   []Record     //qm:行记录数组 cz:Records   []     // Records is the result of query statement.
...
type (
	...
	List   = []Map                   //qm:Map数组 cz:List = []Map     // List is type of map array.
)
