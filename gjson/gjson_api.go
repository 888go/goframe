...
func (j *Json) X加入(表达式 string, 值 interface{}) error {
	...
	return gerror.NewCodef(gcode.CodeInvalidParameter, "invalid variable type of %s", 表达式)
}
...
func (j *Json) X取结构体指针(结构体指针 interface{}, 名称映射 ...map[string]string) error {
	return j.X取泛型类().Scan(结构体指针, 名称映射...)
}
