...
func X是否为有效json(值 interface{}) bool {
	return json.Valid(gconv.Bytes(值))
}
...
func Json格式到变量(值 interface{}, 选项 ...Options) (interface{}, error) {
	var value interface{}
	if err := Json格式到变量指针(gconv.Bytes(值), &value, 选项...); err != nil {
		return nil, err
	} else {
		return value, nil
	}
...
func Json格式到变量指针(值 interface{}, 变量指针 interface{}, 选项 ...Options) (错误 error) {
	decoder := json.NewDecoder(bytes.NewReader(gconv.Bytes(值)))
	if len(选项) > 0 {
...
// 例如，它可能会导致与其他数据格式（如 yaml）转换时出现问题。
	...
	if 错误 = decoder.Decode(变量指针); 错误 != nil {
		错误 = gerror.Wrap(错误, `json Decode failed`)
	}
	return
}
...
func X解码到json(值 interface{}, 选项 ...Options) (*Json, error) {
	if v, err := Json格式到变量(gconv.Bytes(值), 选项...); err != nil {
		return nil, err
	} else {
		if len(选项) > 0 {
