...
func Test_Params_Parse(t *testing.T) {
	...
	s.X绑定("/parseErr", func(r *ghttp.Request) {
		var user User
		err := r.X解析参数到结构(user)
...
func Test_Params_ParseQuery(t *testing.T) {
	...
	s := g.Http类(guid.X生成())
	s.X绑定("/parse-query", func(r *ghttp.Request) {
		var user *User
...
func Test_Params_ParseForm(t *testing.T) {
	...
	s := g.Http类(guid.X生成())
	s.X绑定("/parse-form", func(r *ghttp.Request) {
		var user *User
...
func Test_Params_Parse_Attr_Pointer1(t *testing.T) {
	...
	s := g.Http类(guid.X生成())
	s.X绑定("/parse1", func(r *ghttp.Request) {
		if m := r.GetMap别名(); len(m) > 0 {
			var user *User
			if err := r.X解析参数到结构(&user); err != nil {
				r.X响应.X写响应缓冲区并退出(err)
	...
	s.X绑定("/parse2", func(r *ghttp.Request) {
		if m := r.GetMap别名(); len(m) > 0 {
			var user = new(User)
			if err := r.X解析参数到结构(user); err != nil {
				r.X响应.X写响应缓冲区并退出(err)
...
func Test_Params_Struct(t *testing.T) {
	...
	s := g.Http类(guid.X生成())
	s.X绑定("/struct1", func(r *ghttp.Request) {
		if m := r.GetMap别名(); len(m) > 0 {
			user := new(User)
	...
	s.X绑定("/struct2", func(r *ghttp.Request) {
		if m := r.GetMap别名(); len(m) > 0 {
			user := (*User)(nil)
			if err := r.GetStruct别名(&user); err != nil {
				r.X响应.X写响应缓冲区并退出(err)
	...
	s.X绑定("/struct-valid", func(r *ghttp.Request) {
		if m := r.GetMap别名(); len(m) > 0 {
			user := new(User)
			if err := r.GetStruct别名(user); err != nil {
				r.X响应.X写响应缓冲区并退出(err)
			}
			if err := gvalid.New().Data(user).Run(r.Context别名()); err != nil {
				r.X响应.X写响应缓冲区并退出(err)
			}
		}
	})
