...
func (s *X服务) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// 最大请求体大小限制。
	if s.config.X客户端请求最大长度 > 0 {
		r.Body = http.MaxBytesReader(w, r.Body, s.config.X客户端请求最大长度)
	}
	// 重写特性检查。
	if len(s.config.X路由URI重写规则Map) > 0 {
		if rewrite, ok := s.config.X路由URI重写规则Map[r.URL.Path]; ok {
			r.URL.Path = rewrite
		}
	}
...
// 搜索优先级最高的静态文件，并处理索引文件特性。
	...
	if request.X响应.Status == 0 {
		if request.StaticFile != nil || request.X中间件管理器.served || request.X响应.buffer.Len() > 0 {
			request.X响应.WriteHeader(http.StatusOK)
		} else if err := request.X取错误信息(); err != nil {
			if request.X响应.X取缓冲区长度() == 0 {
...
// 如果在本次请求中创建了新的会话ID，并且启用了SessionCookieOutput，则自动将会话ID设置到cookie中。
	if s.config.SessionID输出到Cookie && request.Session.IsDirty() {
...
func (s *X服务) searchStaticFile(uri string) *staticFile {
	...
	if len(s.config.X静态文件目录映射) > 0 {
		for _, item := range s.config.X静态文件目录映射 {
			if len(uri) >= len(item.Prefix) && strings.EqualFold(item.Prefix, uri[0:len(item.Prefix)]) {
				...
				file = 资源类.GetWithIndex(item.Path+uri[len(item.Prefix):], s.config.X静态文件索引)
				if file != nil {
					return &staticFile{
						File:  file,
				...
				path, dir = 文件搜索类.Search(item.Path, uri[len(item.Prefix):], s.config.X静态文件索引...)
				if path != "" {
					return &staticFile{
						Path:  path,
						IsDir: dir,
	...
	// 其次，搜索根目录和搜索路径。
	if len(s.config.X静态文件额外搜索目录) > 0 {
		for _, p := range s.config.X静态文件额外搜索目录 {
			file = 资源类.GetWithIndex(p+uri, s.config.X静态文件索引)
			if file != nil {
				return &staticFile{
					File:  file,
					IsDir: file.FileInfo().IsDir(),
			...
			if path, dir = 文件搜索类.Search(p, uri, s.config.X静态文件索引...); path != "" {
				return &staticFile{
					Path:  path,
					IsDir: dir,
				}
	...
	// 最后在资源管理器中进行搜索。
	if len(s.config.X静态文件目录映射) == 0 && len(s.config.X静态文件额外搜索目录) == 0 {
		if file = 资源类.GetWithIndex(uri, s.config.X静态文件索引); file != nil {
			return &staticFile{
				File:  file,
				IsDir: file.FileInfo().IsDir(),
			}
...
func (s *X服务) serveFile(r *X请求, f *staticFile, allowIndex ...bool) {
	// 从内存中使用资源文件。
	if f.File != nil {
		if f.IsDir {
			if s.config.X静态文件是否列出子文件 || (len(allowIndex) > 0 && allowIndex[0]) {
				s.listDir(r, f.File)
			} else {
...
func (s *X服务) listDir(r *X请求, f http.File) {
	...
	if r.URL.Path != "/" {
		r.X响应.X写响应缓冲区(`<tr>`)
		r.X响应.X写响应缓冲区并格式化(`<td><a href="%s">..</a></td>`, 文件类.X路径取父目录(r.URL.Path))
		r.X响应.X写响应缓冲区(`</tr>`)
	}
	name := ""
	size := ""
	...
	for _, file := range files {
		...
		r.X响应.X写响应缓冲区(`<tr>`)
		r.X响应.X写响应缓冲区并格式化(`<td><a href="%s/%s">%s</a></td>`, prefix, name, html类.X编码特殊字符(name))
		r.X响应.X写响应缓冲区并格式化(`<td style="width:300px;text-align:center;">%s</td>`, 时间类.X创建(file.ModTime()).X取文本时间ISO8601())
