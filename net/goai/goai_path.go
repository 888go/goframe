...
func (oai *OpenApiV3) addPath(in addPathInput) error {
	...
	var (
		...
		inputMetaMap         = 元数据类.Data(inputObject.Interface())
		outputMetaMap        = 元数据类.Data(outputObject.Interface())
	...
	// Path check.
	if in.Path == "" {
		in.Path = 元数据类.Get(inputObject.Interface(), gtag.Path).String()
		if in.Prefix != "" {
			in.Path = 文本类.X过滤尾字符并含空白(in.Prefix, "/") + "/" + 文本类.X过滤首字符并含空白(in.Path, "/")
		}
	}
	if in.Path == "" {
		return 错误类.X创建错误码并格式化(
	...
	// Method check.
	if in.Method == "" {
		in.Method = 元数据类.Get(inputObject.Interface(), gtag.Method).String()
	}
	if in.Method == "" {
		return 错误类.X创建错误码并格式化(
			错误码类.CodeMissingParameter,
...
// 另外需要注意的是，我们在 Operation 上设置 Summary（摘要）和 Description（描述），而不是在 Path 上设置，所以我们需要将它们移除。
		inputMetaMapForPath := map类.X创建StrStr并从Map(inputMetaMap).X取副本()
...
// 多种模式之间用逗号分隔，例如 `security: apiKey1,apiKey2`
	TagNameSecurity := 元数据类.Get(inputObject.Interface(), gtag.Security).String()
	securities := 文本类.X分割并忽略空值(TagNameSecurity, ",")
...
// =================================================================================================================
	...
	if operation.RequestBody.Value == nil {
		...
		var (
			...
			tagMimeValue = 元数据类.Get(inputObject.Interface(), gtag.Mime).String()
		)
		if tagMimeValue != "" {
			contentTypes = 文本类.X分割并忽略空值(tagMimeValue, ",")
		}
...
// =================================================================================================================
	if _, ok := operation.Responses[responseOkKey]; !ok {
		...
		var (
			contentTypes = oai.Config.ReadContentTypes
			tagMimeValue = 元数据类.Get(outputObject.Interface(), gtag.Mime).String()
	...
	default:
		return 错误类.X创建错误码并格式化(错误码类.CodeInvalidParameter, `invalid method "%s"`, in.Method)
	}
...
func (oai *OpenApiV3) removeOperationDuplicatedProperties(operation Operation) {
	...
	dataFields := 文本类.X分割(oai.Config.CommonRequestDataField, ".")
...
func (oai *OpenApiV3) tagMapToPath(tagMap map[string]string, path *Path) error {
	...
	if err := 转换类.Struct(mergedTagMap, path); err != nil {
		return 错误类.X多层错误(err, `mapping struct tags to Path failed`)
	}
