...
func ExampleCache_Set() {
	...
	// 设置缓存，不设置过期时间
	c.X设置值(ctx, "k1", g.Slice别名{1, 2, 3, 4, 5, 6, 7, 8, 9}, 0)

	// Get cache
	fmt.Println(c.X取值(ctx, "k1"))
...
func ExampleCache_SetIfNotExist() {
	...
	k1, err := c.X设置值并跳过已存在(ctx, "k1", "v1", 1000*time.Millisecond)
	fmt.Println(k1, err)

	// 当键名已存在时返回false
	k2, err := c.X设置值并跳过已存在(ctx, "k1", "v2", 1000*time.Millisecond)
	fmt.Println(k2, err)
...
// 如果 `duration` 等于 0，则不会过期。如果 `duration` 小于 0 或提供的 `value` 为 nil，则会删除 `data` 中的键。
	c.X设置Map(ctx, data, 1000*time.Millisecond)
...
// SetsBatch 函数通过 `data` 设置缓存中的键值对，该键值对在 `duration` 后过期。
// 如果 `duration` 等于 0，则不会过期。如果 `duration` 小于 0 或提供的 `value` 为 nil，则会删除 `data` 中的键。
	c.X设置Map(ctx, g.MapAnyAny{"k1": "v1", "k2": "v2", "k3": "v3"}, 0)
...
func ExampleCache_UpdateExpire() {
	...
	c.X设置值(ctx, "k1", "v1", 1000*time.Millisecond)
	expire, _ := c.X取过期时间(ctx, "k1")
	fmt.Println(expire)

// UpdateExpire 更新键 `key` 的过期时间，并返回旧的过期持续时间值。
// 如果 `key` 不存在于缓存中，则返回 -1 并不做任何操作。
	c.X更新过期时间(ctx, "k1", 500*time.Millisecond)

	expire1, _ := c.X取过期时间(ctx, "k1")
	fmt.Println(expire1)
...
func ExampleCache_Values() {
	...
	// Write value
	c.X设置值(ctx, "k1", g.Map{"k1": "v1", "k2": "v2"}, 0)
...
// Contains 返回 true 如果 `key` 存在于缓存中，否则返回 false。
	data, _ := c.X是否存在(ctx, "k")
	fmt.Println(data)

	// return false
	data1, _ := c.X是否存在(ctx, "k1")
	fmt.Println(data1)
...
func ExampleCache_Data() {
	...
	c.X设置Map(ctx, g.MapAnyAny{"k1": "v1"}, 0)

	data, _ := c.X取所有键值Map副本(ctx)
	fmt.Println(data)
...
// GetOrSet 获取并返回 `key` 对应的值，如果 `key` 不存在于缓存中，则设置 `key`-`value` 键值对并返回 `value`。
	data, _ := c.X取值或设置值(ctx, "k", "v", 10000*time.Millisecond)
	fmt.Println(data)
...
// GetOrSetFunc 函数用于获取并返回 `key` 对应的值，如果 `key` 不存在于缓存中，则使用函数 `f` 的结果设置 `key` 并返回其执行结果。
	c.X取值或设置值_函数(ctx, "k1", func(ctx context.Context) (value interface{}, err error) {
		return "v1", nil
	...
	// 如果函数返回nil，则不执行任何操作
	c.X取值或设置值_函数(ctx, "k2", func(ctx context.Context) (value interface{}, err error) {
		return nil, nil
	}, 10000*time.Millisecond)
	v1, _ := c.X取值(ctx, "k2")
	fmt.Println(v1)
...
func ExampleCache_GetOrSetFuncLock() {
	...
	c.X取值或设置值_并发安全函数(ctx, "k1", func(ctx context.Context) (value interface{}, err error) {
		return "v1", nil
	}, 0)
	v, _ := c.X取值(ctx, "k1")
	fmt.Println(v)
...
func ExampleCache_KeyStrings() {
	...
	c.X设置Map(ctx, g.MapAnyAny{"k1": "v1", "k2": "v2"}, 0)

	// KeyStrings 返回缓存中的所有键，以字符串切片形式。
	keys, _ := c.X取所有键文本(ctx)
	fmt.Println(keys)
...
func ExampleCache_Removes() {
	...
	c.X设置Map(ctx, g.MapAnyAny{"k1": "v1", "k2": "v2", "k3": "v3", "k4": "v4"}, 0)

// Remove 从缓存中删除一个或多个键，并返回其对应的值。
// 如果提供了多个键，它将返回最后被删除项的值。
	c.X删除(ctx, g.Slice别名{"k1", "k2", "k3"})
...
func ExampleCache_MustGetOrSet() {
	...
	k1 := c.X取值或设置值PANI(ctx, "k1", "v1", 0)
	fmt.Println(k1)

	k2 := c.X取值或设置值PANI(ctx, "k1", "v2", 0)
	fmt.Println(k2)
...
func ExampleCache_MustGetOrSetFunc() {
	...
	c.X取值或设置值_函数PANI(ctx, "k1", func(ctx context.Context) (value interface{}, err error) {
		return "v1", nil
	}, 10000*time.Millisecond)
	v := c.X取值PANI(ctx, "k1")
	fmt.Println(v)

	c.X取值或设置值_函数PANI(ctx, "k2", func(ctx context.Context) (value interface{}, err error) {
		return nil, nil
	}, 10000*time.Millisecond)
	v1 := c.X取值PANI(ctx, "k2")
	fmt.Println(v1)
...
func ExampleCache_MustGetOrSetFuncLock() {
	...
	c.X取值或设置值_并发安全函数PANI(ctx, "k1", func(ctx context.Context) (value interface{}, err error) {
		return "v1", nil
	}, 0)
	v := c.X取值PANI(ctx, "k1")
	fmt.Println(v)
...
// MustContains 返回一个布尔值，如果 `key` 存在于缓存中则返回 true，否则返回 false。
	data := c.X是否存在PANI(ctx, "k")
	fmt.Println(data)

	// return false
	data1 := c.X是否存在PANI(ctx, "k1")
	fmt.Println(data1)
...
func ExampleCache_SetAdapter() {
	...
	// 创建Redis客户端对象。
	redis, err := redis类.New(redisConfig)
	if err != nil {
		panic(err)
	}
	// 创建Redis缓存适配器并将它设置为缓存对象。
	cache.X设置适配器(缓存类.NewAdapterRedis(redis))

	// 使用缓存对象进行设置和获取操作。
	err = cache.X设置值(ctx, cacheKey, cacheValue, time.Second)
	if err != nil {
		panic(err)
	}
	fmt.Println(cache.X取值PANI(ctx, cacheKey).String())
...
func ExampleCache_GetAdapter() {
	...
	// 使用redis客户端获取。
	v, err := cache.X取适配器().(*缓存类.AdapterRedis).X取值(ctx, cacheKey)
	fmt.Println(err)
