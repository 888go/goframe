...
var (
	...
	pathsMap = gmap.X创建StrAny(true)
)

// New 创建并返回一个新的路径搜索管理器。 md5:4a9d5d03b9c2c8be
func New(path string, cache bool) *SPath {
	sp := &SPath{
		paths: garray.X创建文本(true),
	}
	if cache {
		sp.cache = gmap.X创建StrStr(true)
	}
	if len(path) > 0 {
...
func (sp *SPath) X设置值(path string) (realPath string, err error) {
	...
	if realPath == "" {
		...
		if realPath == "" {
			realPath = gfile.X取绝对路径且效验(gfile.X取当前工作目录() + gfile.Separator + path)
		}
	}
	if realPath == "" {
		return realPath, gerror.X创建错误码并格式化(gcode.CodeInvalidParameter, `path "%s" does not exist`, path)
	}
		// 设置的路径必须是一个目录。 md5:a1d52d7d0583a6ef
	if gfile.X是否存在目录(realPath) {
		...
		return "", gerror.X创建错误码(gcode.CodeInvalidParameter, path+" should be a folder")
	}
}
...
func (sp *SPath) Search(name string, indexFiles ...string) (filePath string, isDir bool) {
	...
	if sp.cache == nil {
		sp.paths.X遍历写锁定(func(array []string) {
			path := ""
			for _, v := range array {
				path = gfile.X路径生成(v, name)
				if stat, err := os.Stat(path); stat != nil && !os.IsNotExist(err) {
					path = gfile.X取绝对路径(path)
