...
func Test_Copy_File_To_Dir(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		var (
			src = 单元测试类.DataPath("dir1", "file1")
			dst = 文件类.X取临时目录(uid类.X生成(), "dir2")
		)
		err := 文件类.X创建目录(dst)
		t.AssertNil(err)
		defer 文件类.X删除(dst)
...
func Test_Copy_Dir_To_File(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		var (
			src = 单元测试类.DataPath("dir1")
			dst = 文件类.X取临时目录(uid类.X生成(), "file2")
		)
...
func Test_CopyDir(t *testing.T) {
	...
	// 内容替换
	单元测试类.C(t, func(t *单元测试类.T) {
		src := 文件类.X取临时目录(时间类.X取文本时间戳纳秒(), 时间类.X取文本时间戳纳秒())
		dst := 文件类.X取临时目录(时间类.X取文本时间戳纳秒(), 时间类.X取文本时间戳纳秒())
		defer func() {
			文件类.X删除(src)
			文件类.X删除(dst)
		}()
