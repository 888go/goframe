...
type Cron struct {
	idGen   *安全变量类.Int64    // 用于生成唯一名称。
	status  *安全变量类.Int      // 定时任务状态(0: 未开始; 1: 运行中; 2: 已停止; -1: 关闭)
	entries *map类.StrAnyMap // 所有定时任务条目。
	logger  日志类.ILogger    // Logger，默认情况下为nil。
}

// New 函数返回一个使用默认设置的新 Cron 对象。
func New() *Cron {
	return &Cron{
		idGen:   安全变量类.NewInt64(),
		status:  安全变量类.NewInt(StatusRunning),
		entries: map类.X创建StrAny(true),
	}
}
...
func (c *Cron) DelayAddEntry(ctx context.Context, delay time.Duration, pattern string, job JobFunc, times int, isSingleton bool, name ...string) {
	定时类.X加入单次任务(ctx, delay, func(ctx context.Context) {
...
func (c *Cron) Entries() []*Entry {
	array := 数组类.X创建排序并按大小(c.entries.X取数量(), func(v1, v2 interface{}) int {
		entry1 := v1.(*Entry)
		entry2 := v2.(*Entry)
