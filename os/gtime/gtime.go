...
func X转换文本(文本时间 string, 格式 ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(文本时间); len(match) > 0 && match[1] != "" {
		...
		return nil, 错误类.X创建错误码并格式化(错误码类.CodeInvalidParameter, `unsupported time converting for string "%s"`, 文本时间)
	}
...
// 则进行时区转换操作，将时间转换为 UTC 格式。
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, 错误类.X创建错误码并格式化(错误码类.CodeInvalidParameter, `invalid zone string "%s"`, match[6])
			}
			operation := match[5]
...
// 如果不相等，则将其转换为UTC。
	...
	if month <= 0 || day <= 0 {
		return nil, 错误类.X创建错误码并格式化(错误码类.CodeInvalidParameter, `invalid time string "%s"`, 文本时间)
	}
...
func X转换时区(文本时间 string, 新时区 string, 旧时区 ...string) (*Time, error) {
	...
	if len(旧时区) > 0 {
		if l, err = time.LoadLocation(旧时区[0]); err != nil {
			err = 错误类.X多层错误码并格式化(错误码类.CodeInvalidParameter, err, `time.LoadLocation failed for name "%s"`, 旧时区[0])
			return nil, err
		} else {
	...
	if l, err = time.LoadLocation(新时区); err != nil {
		err = 错误类.X多层错误码并格式化(错误码类.CodeInvalidParameter, err, `time.LoadLocation failed for name "%s"`, 新时区)
		return nil, err
	} else {
...
func X转换文本Layout(文本时间 string, 格式 string) (*Time, error) {
	if t, err := time.ParseInLocation(格式, 文本时间, time.Local); err == nil {
		...
		return nil, 错误类.X多层错误码并格式化(
			错误码类.CodeInvalidParameter, err,
...
func X解析文本(文本 string, 格式 ...string) *Time {
	...
	if len(格式) > 0 {
		for _, item := range 格式 {
			match, err = 正则类.X匹配文本(formatToRegexPattern(item), 文本)
			if err != nil {
...
func X文本取时长(文本 string) (纳秒 time.Duration, 错误 error) {
	...
	if utils.IsNumeric(文本) {
		...
		if 错误 != nil {
			错误 = 错误类.X多层错误码并格式化(错误码类.CodeInvalidParameter, 错误, `strconv.ParseInt failed for string "%s"`, 文本)
			return 0, 错误
		}
		return time.Duration(num), nil
	}
	match, 错误 := 正则类.X匹配文本(`^([\-\d]+)[dD](.*)$`, 文本)
	if 错误 != nil {
		return 0, 错误
	}
	if len(match) == 3 {
		...
		if 错误 != nil {
			错误 = 错误类.X多层错误码并格式化(错误码类.CodeInvalidParameter, 错误, `strconv.ParseInt failed for string "%s"`, match[1])
			return 0, 错误
		}
		文本 = fmt.Sprintf(`%dh%s`, num*24, match[2])
		纳秒, 错误 = time.ParseDuration(文本)
		if 错误 != nil {
			错误 = 错误类.X多层错误码并格式化(错误码类.CodeInvalidParameter, 错误, `time.ParseDuration failed for string "%s"`, 文本)
		}
		return
	}
	纳秒, 错误 = time.ParseDuration(文本)
