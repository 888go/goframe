...
func Zip压缩文件(目录或文件, 压缩文件路径 string, 可选路径前缀 ...string) error {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `os.Create failed for name "%s"`, 压缩文件路径)
		return err
	}
	defer writer.Close()
	...
	for _, path := range strings.Split(目录或文件, ",") {
		...
		if err = doZipPathWriter(path, 文件类.X取绝对路径且效验(压缩文件路径), zipWriter, 可选路径前缀...); err != nil {
			return err
		}
	}
	return nil
...
func doZipPathWriter(fileOrFolderPath string, exclude string, zipWriter *zip.Writer, prefix ...string) error {
	...
	fileOrFolderPath, err = 文件类.X查找(fileOrFolderPath)
	if err != nil {
		return err
	}
	if 文件类.X是否存在目录(fileOrFolderPath) {
		files, err = 文件类.X枚举并含子目录名(fileOrFolderPath, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if 文件类.X是否存在目录(fileOrFolderPath) {
		if len(headerPrefix) > 0 {
			...
			headerPrefix = 文件类.X路径取文件名(fileOrFolderPath)
		}
	}
	...
	for _, file := range files {
		...
		dir := 文件类.X路径取父目录(file[len(fileOrFolderPath):])
		if dir == "." {
			dir = ""
		}
...
func Zip解压文件(压缩包路径, 解压目录 string, 可选路径前缀 ...string) error {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `zip.OpenReader failed for name "%s"`, 解压目录)
		return err
	}
...
func Zip解压字节集(zip字节集 []byte, 解压目录 string, 可选路径前缀 ...string) error {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `zip.NewReader failed`)
		return err
	}
...
func unZipFileWithReader(reader *zip.Reader, dstFolderPath string, zippedPrefix ...string) error {
	...
	for _, file := range reader.File {
		...
		if len(dstDir) > 0 {
			if _, err := os.Stat(dstDir); os.IsNotExist(err) {
				if err = os.MkdirAll(dstDir, 0755); err != nil {
					err = 错误类.X多层错误并格式化(err, `os.MkdirAll failed for path "%s"`, dstDir)
					return err
				}
			}
		}
		...
		if err != nil {
			err = 错误类.X多层错误并格式化(err, `file.Open failed`)
			return err
		}
...
func doCopyForUnZipFileWithReader(file *zip.File, fileReader io.ReadCloser, dstPath string) error {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `os.OpenFile failed for name "%s"`, dstPath)
		return err
	}
	defer targetFile.Close()

	if _, err = io.Copy(targetFile, fileReader); err != nil {
		err = 错误类.X多层错误并格式化(err, `io.Copy failed from "%s" to "%s"`, file.Name, dstPath)
		return err
	}
	return nil
}
...
func zipFile(filePath string, prefix string, zw *zip.Writer) error {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `os.Open failed for name "%s"`, filePath)
		return err
	}
	defer file.Close()
	...
	info, err := file.Stat()
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `file.Stat failed for name "%s"`, filePath)
		return err
	}
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `zip.Writer.CreateHeader failed for header "%#v"`, header)
		return err
	}
	if !info.IsDir() {
		if _, err = io.Copy(writer, file); err != nil {
			err = 错误类.X多层错误并格式化(err, `io.Copy failed from "%s" to "%s"`, filePath, header.Name)
			return err
		}
	}
	return nil
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if err != nil {
		err = 错误类.X多层错误并格式化(err, `zip.FileInfoHeader failed for info "%#v"`, info)
		return nil, err
	}
