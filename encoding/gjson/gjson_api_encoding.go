...
func (j *Json) X取xml字节集(rootTag ...string) ([]byte, error) {
	return xml类.Encode(j.X取泛型类().X取Map(), rootTag...)
}
...
func (j *Json) X取xml字节集并格式化(rootTag ...string) ([]byte, error) {
	return xml类.EncodeWithIndent(j.X取泛型类().X取Map(), rootTag...)
}
...
func (j *Json) X取YAML字节集() ([]byte, error) {
	j.mu.RLock()
	defer j.mu.RUnlock()
	return yaml类.Encode(*(j.p))
}

func (j *Json) X取YAML字节集并格式化(缩进 string) ([]byte, error) {
	j.mu.RLock()
	defer j.mu.RUnlock()
	return yaml类.EncodeIndent(*(j.p), 缩进)
}
