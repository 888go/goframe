...
var (
	...
	content_16, _    = 编码base64类.X文本解码到字节集("v1jqsGHId/H8onlVHR8Vaw==")
	content_24, _    = 编码base64类.X文本解码到字节集("0TXOaj5KMoLhNWmJ3lxY1A==")
	content_32, _    = 编码base64类.X文本解码到字节集("qM/Waw1kkWhrwzek24rCSA==")
	content_16_iv, _ = 编码base64类.X文本解码到字节集("DqQUXiHgW/XFb6Qs98+hrA==")
	content_32_iv, _ = 编码base64类.X文本解码到字节集("ZuLgAOii+lrD5KJoQ7yQ8Q==")
	...
	content_16_cfb, _ = 编码base64类.X文本解码到字节集("oSmget3aBDT1nJnBp8u6kA==")
)

func TestEncrypt(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		data, err := 加密aes类.Encrypt别名(content, key_16)
		t.AssertNil(err)
		t.Assert(data, []byte(content_16))
		data, err = 加密aes类.Encrypt别名(content, key_24)
		t.AssertNil(err)
		t.Assert(data, []byte(content_24))
		data, err = 加密aes类.Encrypt别名(content, key_32)
		t.AssertNil(err)
		t.Assert(data, []byte(content_32))
		data, err = 加密aes类.Encrypt别名(content, key_16, iv)
		t.AssertNil(err)
		t.Assert(data, []byte(content_16_iv))
		data, err = 加密aes类.Encrypt别名(content, key_32, iv)
		t.AssertNil(err)
...
func TestDecrypt(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		decrypt, err := 加密aes类.Decrypt别名([]byte(content_16), key_16)
		t.AssertNil(err)
		t.Assert(decrypt, content)

		decrypt, err = 加密aes类.Decrypt别名([]byte(content_24), key_24)
		t.AssertNil(err)
		t.Assert(decrypt, content)

		decrypt, err = 加密aes类.Decrypt别名([]byte(content_32), key_32)
		t.AssertNil(err)
		t.Assert(decrypt, content)

		decrypt, err = 加密aes类.Decrypt别名([]byte(content_16_iv), key_16, iv)
		t.AssertNil(err)
		t.Assert(decrypt, content)

		decrypt, err = 加密aes类.Decrypt别名([]byte(content_32_iv), key_32, iv)
		t.AssertNil(err)
		t.Assert(decrypt, content)

		decrypt, err = 加密aes类.Decrypt别名([]byte(content_32_iv), keys, iv)
...
func TestEncryptErr(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		// encrypt key error
		_, err := 加密aes类.Encrypt别名(content, key_err)
		t.AssertNE(err, nil)
	})
}
...
func TestDecryptErr(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		// decrypt key error
		encrypt, err := 加密aes类.Encrypt别名(content, key_16)
		_, err = 加密aes类.Decrypt别名(encrypt, key_err)
		t.AssertNE(err, nil)

		// 解密内容过短错误
		_, err = 加密aes类.Decrypt别名([]byte("test"), key_16)
		t.AssertNE(err, nil)

		// 解密内容大小错误
		_, err = 加密aes类.Decrypt别名(key_17, key_16)
		t.AssertNE(err, nil)
	})
}
...
// 此函数通常用于对使用PKCS#5填充方式加密的数据进行解密前的填充剥离
		_, err := 加密aes类.PKCS5UnPadding(content, 0)
		t.AssertNE(err, nil)
...
// 在密码学中，PKCS5Padding是对明文进行填充以满足块加密算法对输入长度要求的一种方法。在解密时需要将这些填充的部分去除。上述注释表明这是一个用于在解密后执行PKCS5填充去除操作的函数，并特别指出了当输入长度为0时的情况，这可能意味着原始数据不包含有效的填充或已经不是完整的块。
		_, err = 加密aes类.PKCS5UnPadding([]byte(""), 16)
		t.AssertNE(err, nil)
...
// 这段注释表明这是一个Go语言函数，用于执行PKCS#5填充的解填充操作，前提是源数据的长度大于块大小。在密码学中，PKCS#5填充是一种用于确保明文数据长度适配分组密码（如AES）所需块大小的方法，在解密后需要进行解填充以恢复原始数据。
		_, err = 加密aes类.PKCS5UnPadding(key_17, 16)
		t.AssertNE(err, nil)

		// PKCS5UnPadding 当原始数据长度大于块大小时
// 这段注释表明这是一个Go语言函数，用于执行PKCS#5填充的解填充操作，前提是源数据的长度大于块大小。在密码学中，PKCS#5填充是一种用于确保明文数据长度适配分组密码（如AES）所需块大小的方法，在解密后需要进行解填充以恢复原始数据。
		_, err = 加密aes类.PKCS5UnPadding(key_32_err, 32)
		t.AssertNE(err, nil)
	})
...
// func PKCS7UnPadding(paddedData []byte, blockSize int) ([]byte, error) {
    // ...
// }
		_, err := 加密aes类.PKCS7UnPadding(content, 0)
		t.AssertNE(err, nil)

		// PKCS7UnPadding 对PKCS7填充方式进行解码，src为待解码的源数据，len表示源数据长度，需要注意的是这里的源数据长度可能为零。
		_, err = 加密aes类.PKCS7UnPadding([]byte(""), 16)
		t.AssertNE(err, nil)
...
// 在密码学中，PKCS7Padding是一种常用的对称加密数据填充方式，确保加密后数据长度是块大小的整数倍。当解密时，需要通过PKCS7UnPadding函数去除末尾的填充数据。这段注释说明了该函数的作用和调用前的约束条件——待解填充的数据长度必须大于块大小。
		_, err = 加密aes类.PKCS7UnPadding(key_17, 16)
		t.AssertNE(err, nil)

		// PKCS7UnPadding 函数，用于处理src（源数据）的PKCS#7解填充，要求src的长度大于块大小（blockSize）
// 在密码学中，PKCS7Padding是一种常用的对称加密数据填充方式，确保加密后数据长度是块大小的整数倍。当解密时，需要通过PKCS7UnPadding函数去除末尾的填充数据。这段注释说明了该函数的作用和调用前的约束条件——待解填充的数据长度必须大于块大小。
		_, err = 加密aes类.PKCS7UnPadding(key_32_err, 32)
		t.AssertNE(err, nil)
	})
}
...
func TestEncryptCFB(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		var padding int = 0
		data, err := 加密aes类.X加密CFB(content, key_16, &padding, iv)
		t.AssertNil(err)
...
func TestDecryptCFB(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		decrypt, err := 加密aes类.X解密CFB([]byte(content_16_cfb), key_16, padding_size, iv)
		t.AssertNil(err)
		t.Assert(decrypt, content)
	})
}
