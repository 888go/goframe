...
// 2. 结果大于等于0，但小于 `max`；
// 3. 结果数字是32位的，并且小于 math.MaxUint32。 md5:a6e33dbab8d41e7e
...
// N 返回一个在`min`和`max`之间的随机整数：[min, max]。`min`和`max`也支持负数。 md5:7512aa05c9a381dc
func N(min, max int) int {
	...
	// 所以我们首先需要将值向右移动，
	// 然后调用`Intn`生成随机数，
	// 最后再将结果向左移动回去。 md5:4ce39d9c2b1e67a4
	return Intn(max+(0-min)+1) - (0 - min)
}

// S 返回一个随机字符串，该字符串包含数字和字母，其长度为 `n`。
// 可选参数 `symbols` 指定结果中是否可以包含符号，默认为 false。 md5:9b705df69354173e
...
// Str 随机从给定字符串 `s` 中选择并返回 `n` 个字符。它也支持Unicode字符串，如中文、俄文、日文等。 md5:49cb0972916cd378
...
// Perm返回一个整数切片，它是[0,n)范围内的伪随机排列。该排列作为长度为n的int类型的元素组成。// TODO：优化对于生成大切片时的性能。 md5:2a47a879b72568c7
func Perm(n int) []int {
	m := make([]int, n)
	for i := 0; i < n; i++ {
		j := Intn(i + 1)
