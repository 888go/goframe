...
type iBytes interface {
	Bytes() []byte //qm:取字节集 cz:Bytes() []byte     
}
...
type iSet interface {
	Set(value interface{}) (old interface{}) //qm:设置值 cz:Set(     
}

// iGTime is the interface for gtime.Time converting.
type iGTime interface {
	GTime(format ...string) *gtime.Time //qm:取gtime时间类 cz:GTime(     
}
