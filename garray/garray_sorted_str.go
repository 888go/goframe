...
func (a *SortedStrArray) X遍历写锁定(回调函数 func(切片 []string)) *SortedStrArray {
	a.mu.Lock()
	defer a.mu.Unlock()
	回调函数(a.array)
	return a
...
func (a *SortedStrArray) X遍历读锁定(回调函数 func(切片 []string)) *SortedStrArray {
	a.mu.RLock()
	defer a.mu.RUnlock()
	回调函数(a.array)
	return a
...
func (a *SortedStrArray) X合并(切片 interface{}) *SortedStrArray {
	return a.X入栈右(gconv.Strings(切片)...)
}
