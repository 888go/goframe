...
// NewArraySize 根据给定的大小和容量创建并返回一个切片。
...
// NewArrayRange 根据指定的范围从 `start` 到 `end` 并以步长值 `step` 创建并返回一个切片。
...
// NewArrayFrom 通过给定的切片 `array` 创建并返回一个切片。
// 参数 `safe` 用于指定是否在并发安全的情况下使用切片，默认为 false。
...
// NewArrayFromCopy 通过复制给定切片 `array` 创建并返回一个切片。
// 参数 `safe` 用于指定是否在并发安全的情况下使用切片，默认为 false。
...
func (a *Array) X遍历并写锁定(回调函数 func(切片 []interface{})) *Array {
	a.mu.Lock()
	defer a.mu.Unlock()
	回调函数(a.array)
	return a
...
func (a *Array) X遍历并读锁定(回调函数 func(切片 []interface{})) *Array {
	a.mu.RLock()
	defer a.mu.RUnlock()
	回调函数(a.array)
	return a
...
func (a *Array) X合并(切片 interface{}) *Array {
	return a.Append别名(gconv.Interfaces(切片)...)
}
