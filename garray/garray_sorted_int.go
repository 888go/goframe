...
// SortedIntArray 是一个具有丰富功能的Golang有序整数切片。
...
// NewSortedIntArraySize 根据给定的大小和容量创建并返回一个已排序的整数切片。
// 参数`safe`用于指定是否在并发安全的情况下使用切片，默认为false。
...
// NewSortedIntArrayRange 根据给定的范围从 `start` 到 `end`，并以指定的步长 `step` 创建并返回一个已排序的整数切片。
...
// NewSortedIntArrayFrom 根据给定的切片 `array` 创建并返回一个已排序的切片。
...
// NewSortedIntArrayFromCopy 通过复制给定切片`array`创建并返回一个已排序的切片。
// 参数`safe`用于指定是否在并发安全的情况下使用切片，默认为false。
// ```go
// NewSortedIntArrayFromCopy 函数通过复制给定的切片 `array`，生成并返回一个新的已排序整数切片。
// 其中参数 `safe` 表示是否需要在并发安全模式下使用该切片，默认情况下为不安全（false）。
...
func (a *SortedIntArray) X遍历写锁定(回调函数 func(切片 []int)) *SortedIntArray {
	a.mu.Lock()
	defer a.mu.Unlock()
	回调函数(a.array)
	return a
...
func (a *SortedIntArray) X遍历读锁定(回调函数 func(切片 []int)) *SortedIntArray {
	a.mu.RLock()
	defer a.mu.RUnlock()
	回调函数(a.array)
	return a
