...
func TestIntArray_PopLeft(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		array := 数组类.X创建整数并从数组(g.SliceInt{1, 2, 3})
		v, ok := array.X出栈左()
		t.Assert(v, 1)
		t.Assert(ok, true)
...
func TestIntArray_Filter(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		array := 数组类.X创建整数并从数组(g.SliceInt{0, 1, 2, 3, 4, 0})
		t.Assert(array.X遍历删除(func(index int, value int) bool {
			return empty.IsEmpty(value)
		}), g.SliceInt{1, 2, 3, 4})
	})
	gtest.C(t, func(t *gtest.T) {
		array := 数组类.X创建整数并从数组(g.SliceInt{1, 2, 3, 4})
		t.Assert(array.X遍历删除(func(index int, value int) bool {
			return empty.IsEmpty(value)
...
func TestIntArray_Walk(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		array := 数组类.X创建整数并从数组(g.SliceInt{1, 2})
