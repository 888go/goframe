...
func ExampleNewIntArrayFrom() {
	s := 数组类.X创建整数并从数组(g.SliceInt{10, 20, 15, 30})
	fmt.Println(s.X取切片(), s.X取长度(), cap(s.X取切片()))
...
func ExampleNewIntArrayFromCopy() {
	s := 数组类.X创建整数并从数组复制(g.SliceInt{10, 20, 15, 30})
	fmt.Println(s.X取切片(), s.X取长度(), cap(s.X取切片()))
...
func ExampleIntArray_SetArray() {
	s := 数组类.X创建整数()
	s.X设置数组(g.SliceInt{10, 20, 15, 30})
	fmt.Println(s.X取切片())
...
func ExampleIntArray_PopRand() {
	s := 数组类.X创建整数()
	s.X设置数组(g.SliceInt{10, 20, 15, 30, 40, 50, 60, 70})
	fmt.Println(s)
	r, _ := s.X出栈随机()
	fmt.Println(s)
	fmt.Println(r)
...
func ExampleIntArray_PopRands() {
	s := 数组类.X创建整数()
	s.X设置数组(g.SliceInt{10, 20, 15, 30, 40, 50, 60})
	fmt.Println(s)
	r := s.X出栈随机多个(2)
	fmt.Println(s)
	fmt.Println(r)
...
func ExampleIntArray_Unique() {
	s := 数组类.X创建整数()
	s.X设置数组(g.SliceInt{10, 20, 15, 15, 20, 50, 60})
	fmt.Println(s)
	fmt.Println(s.X去重())
...
func ExampleIntArray_LockFunc() {
	s := 数组类.X创建整数并从数组(g.SliceInt{10, 20, 15, 30, 40, 50, 60})
...
func ExampleIntArray_Merge() {
	s1 := 数组类.X创建整数()
	s2 := 数组类.X创建整数()
	s1.X设置数组(g.SliceInt{10, 20, 15})
	s2.X设置数组(g.SliceInt{40, 50, 60})
	fmt.Println(s1)
	fmt.Println(s2)
	s1.X合并(s2)
	fmt.Println(s1)
...
func ExampleIntArray_CountValues() {
	s := 数组类.X创建整数并从数组(g.SliceInt{10, 20, 15, 15, 40, 40, 40})
	fmt.Println(s.X统计())
...
func ExampleIntArray_MarshalJSON() {
	...
	var array 数组类.IntArray
	array.X设置数组(g.SliceInt{98, 97, 96})
	s := Student{
		Id:     1,
		Name:   "john",
		Scores: array,
...
func ExampleIntArray_Filter() {
	array1 := 数组类.X创建整数并从数组(g.SliceInt{10, 40, 50, 0, 0, 0, 60})
	array2 := 数组类.X创建整数并从数组(g.SliceInt{10, 4, 51, 5, 45, 50, 56})
...
func ExampleIntArray_FilterEmpty() {
	s := 数组类.X创建整数并从数组(g.SliceInt{10, 40, 50, 0, 0, 0, 60})
	fmt.Println(s)
