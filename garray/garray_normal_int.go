...
// NewIntArrayRange 根据指定范围从 `start` 到 `end` 以步长值 `step` 创建并返回一个整数切片。
...
// NewIntArrayFrom 根据给定的切片 `array` 创建并返回一个切片。
// 参数 `safe` 用于指定是否在并发安全的情况下使用切片，默认为 false。
...
// NewIntArrayFromCopy 通过复制给定切片 `array` 创建并返回一个切片。
// 参数 `safe` 用于指定是否在并发安全的情况下使用切片，默认为 false。
...
func (a *IntArray) X遍历写锁定(回调函数 func(切片 []int)) *IntArray {
	a.mu.Lock()
	defer a.mu.Unlock()
	回调函数(a.array)
	return a
...
func (a *IntArray) X遍历读锁定(回调函数 func(切片 []int)) *IntArray {
	a.mu.RLock()
	defer a.mu.RUnlock()
	回调函数(a.array)
	return a
...
func (a *IntArray) X合并(切片 interface{}) *IntArray {
	return a.Append别名(gconv.Ints(切片)...)
}
