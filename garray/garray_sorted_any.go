...
func (a *SortedArray) X遍历写锁定(回调函数 func(切片 []interface{})) *SortedArray {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *SortedArray) X遍历读锁定(回调函数 func(切片 []interface{})) *SortedArray {
	a.mu.RLock()
	defer a.mu.RUnlock()
	回调函数(a.array)
	return a
...
func (a *SortedArray) X合并(切片 interface{}) *SortedArray {
	return a.X入栈右(gconv.Interfaces(切片)...)
}
