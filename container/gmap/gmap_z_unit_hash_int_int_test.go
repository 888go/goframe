...
func Test_IntIntMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := gmap.X创建IntInt并从Map(map[int]int{1: 1, 2: 2})
		t.Assert(m2.X取Map(), map[int]int{1: 1, 2: 2})
	})

	gtest.C(t, func(t *gtest.T) {
		m := gmap.X创建IntInt(true)
		m.X设置值(1, 1)
...
func Test_IntIntMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		// clone 方法是深克隆
		m := gmap.X创建IntInt并从Map(map[int]int{1: 1, 2: 2})
...
func Test_IntIntMap_Pop(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		k1, v1 := m.X出栈()
		t.AssertIN(k1, g.Slice别名{1, 2})
		t.AssertIN(v1, g.Slice别名{11, 22})
		t.Assert(m.X取数量(), 1)
		k2, v2 := m.X出栈()
		t.AssertIN(k2, g.Slice别名{1, 2})
...
func Test_IntIntMap_IsSubOf(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m1 := gmap.X创建IntAny并从Map(g.MapIntAny{
			1: 1,
			2: 2,
		})
		m2 := gmap.X创建IntAny并从Map(g.MapIntAny{
			2: 2,
		})
...
func Test_IntIntMap_Diff(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m1 := gmap.X创建IntInt并从Map(g.MapIntInt{
			0: 0,
			1: 1,
			2: 2,
			3: 3,
		})
		m2 := gmap.X创建IntInt并从Map(g.MapIntInt{
			0: 0,
			2: 2,
			3: 31,
			4: 4,
