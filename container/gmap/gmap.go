...
func X创建(并发安全 ...bool) *Map {
	return X创建AnyAny(并发安全...)
}

// X创建并从Map 根据给定的映射 `data` 创建并返回一个哈希映射。
...
func X创建并从Map(map值 map[interface{}]interface{}, 并发安全 ...bool) *Map {
	return X创建AnyAny并从Map(map值, 并发安全...)
}

// NewHashMap别名 创建并返回一个空的哈希映射。
...
// NewHashMapFrom别名 从给定的映射 `data` 创建并返回一个哈希映射。
