...
func Test_IntStrMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := gmap.X创建IntStr并从Map(map[int]string{1: "a", 2: "b"})
		t.Assert(m2.X取Map(), map[int]string{1: "a", 2: "b"})
	})

	gtest.C(t, func(t *gtest.T) {
		m := gmap.X创建IntStr(true)
		m.X设置值(1, "val1")
...
func Test_IntStrMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		// clone 方法是深克隆
		m := gmap.X创建IntStr并从Map(map[int]string{1: "a", 2: "b", 3: "c"})
...
func Test_IntStrMap_Pop(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m := gmap.X创建IntStr并从Map(g.MapIntStr{
			1: "v1",
			2: "v2",
		})
		t.Assert(m.X取数量(), 2)
...
func Test_IntStrMap_IsSubOf(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m1 := gmap.X创建IntStr并从Map(g.MapIntStr{
			1: "v1",
			2: "v2",
		})
		m2 := gmap.X创建IntStr并从Map(g.MapIntStr{
			2: "v2",
		})
