...
func Test_AnyAnyMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := gmap.X创建AnyAny并从Map(map[interface{}]interface{}{1: 1, 2: "2"})
...
func Test_AnyAnyMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		// clone 方法是深克隆
		m := gmap.X创建AnyAny并从Map(map[interface{}]interface{}{1: 1, 2: "2"})
...
func Test_AnyAnyMap_IsSubOf(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		m1 := gmap.X创建AnyAny并从Map(g.MapAnyAny{
			"k1": "v1",
			"k2": "v2",
		})
		m2 := gmap.X创建AnyAny并从Map(g.MapAnyAny{
			"k2": "v2",
		})
