...
func Test_IntAnyMap_Basic(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		m2 := gmap.X创建IntAny并从Map(map[int]interface{}{1: 1, 2: "2"})
		t.Assert(m2.X取Map(), map[int]interface{}{1: 1, 2: "2"})
	})

	gtest.C(t, func(t *gtest.T) {
		m := gmap.X创建IntAny(true)
		m.X设置值(1, 1)
...
func Test_IntAnyMap_Clone(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		// clone 方法是深克隆
		m := gmap.X创建IntAny并从Map(map[int]interface{}{1: 1, 2: "2"})
