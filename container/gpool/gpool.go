...
type Pool struct {
	...
	// 例如：net.Conn、os.File 等。 md5:f09911de2780aeaa
...
// ttl = 0：未过期；
// ttl < 0：使用后立即过期；
// ttl > 0：超时过期。 md5:9f724382dd2313e7
...
func (p *Pool) Put(value interface{}) error {
	...
	if p.TTL == 0 {
		...
		// 因此我们需要使用其纳秒值来计算毫秒数。 md5:87b516a9573fac98
...
// 则会使用NewFunc创建并返回一个项目。 md5:7782b49d380b807b
...
// Close 关闭池。如果 `p` 有 ExpireFunc，那么在关闭之前，它会自动使用这个函数关闭所有项目。通常情况下，你不需要手动调用这个函数。 md5:368c18d44115f9cc
func (p *Pool) Close() {
	p.closed.Set(true)
}
...
func (p *Pool) checkExpireItems(ctx context.Context) {
	if p.closed.Val() {
		// 如果p具有ExpireFunc，
		// 则必须使用此函数关闭所有项。 md5:8ec38193671c9632
		if p.ExpireFunc != nil {
			for {
				if r := p.list.PopFront(); r != nil {
					p.ExpireFunc(r.(*poolItem).value)
	...
	// 对于每个项目过期的判断不是非常精确，但性能较高。 md5:5dc686eec927131e
