...
func Test_AVLTree_Basic(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		...
		m2 := 树形类.NewAVLTreeFrom(工具类.X比较文本, map[interface{}]interface{}{1: 1, "key1": "val1"})
...
func Test_AVLTree_Iterator(t *testing.T) {
	...
	m := 树形类.NewAVLTreeFrom(工具类.X比较文本, expect)
...
func Test_AVLTree_Clone(t *testing.T) {
	单元测试类.C(t, func(t *单元测试类.T) {
		//clone 方法是深克隆
		m := 树形类.NewAVLTreeFrom(工具类.X比较文本, map[interface{}]interface{}{1: 1, "key1": "val1"})
		m_clone := m.Clone()
		m.Remove(1)
...
func Test_AVLTree_LRNode(t *testing.T) {
	...
	//unsafe
	单元测试类.C(t, func(t *单元测试类.T) {
		m := 树形类.NewAVLTreeFrom(工具类.X比较文本, expect, true)
		t.Assert(m.Left().Key, "key1")
		t.Assert(m.Right().Key, "key4")
	})
}
...
func Test_AVLTree_CeilingFloor(t *testing.T) {
	...
	//found and eq
	单元测试类.C(t, func(t *单元测试类.T) {
		m := 树形类.NewAVLTreeFrom(工具类.X比较整数, expect)
		c, cf := m.Ceiling(8)
		t.Assert(cf, true)
