...
// X是否存在并忽略大小写 检查集合中是否存在某个值（忽略大小写）。
...
func (set *StrSet) String() string {
	...
	buffer.WriteByte('[')
	for k := range set.data {
		buffer.WriteString(`"` + gstr.X转义并按字符(k, `"\`) + `"`)
		if i != l-1 {
			buffer.WriteByte(',')
		}
		i++
...
func (set *StrSet) X遍历修改(f func(item string) string) *StrSet {
	set.mu.Lock()
	defer set.mu.Unlock()
